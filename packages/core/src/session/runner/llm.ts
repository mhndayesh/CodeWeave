        ...
        Stream.runForEach((event) =>
          Effect.gen(function* () {
            ...
            yield* Effect.uninterruptibleMask((restore) =>
              Effect.sync(() => LiveContext.snapshot(location.directory)).pipe(
                Effect.flatMap((before) =>
                  restore(
                    toolMaterialization
                      ...
                      .pipe(
                        Effect.tap(() =>
                          LiveContext.invalidateChanged(
                            location.directory,
                            before,
                            LiveContext.snapshot(location.directory),
                          ).pipe(Effect.ignore),
                        ),
                      ),
                  ),
...
function latestUserText(messages: ReadonlyArray<{ readonly type: string; readonly text?: string }>) {
  for (let i = messages.length - 1; i >= 0; i--) {
    const message = messages[i]
    if (message?.type === "user" && message.text?.trim()) return message.text
  }
  return undefined
}
