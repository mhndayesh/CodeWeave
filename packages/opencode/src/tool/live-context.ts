...
// NOTE: init must stay requirement-free. Tool inits are evaluated during instance
// load, before InstanceRef is provided, so reading per-instance config here (via
// opencode's Config.Service, which is InstanceState-backed) dies with "InstanceRef
// not provided" and breaks every command + the TUI. The `liveContextCompiler`
// config block is honored by the core builtin tool surface (packages/core/src/tool/
// live-context.ts), which reads location-scoped config that needs no InstanceRef.
